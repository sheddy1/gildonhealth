@section('about')
    <div class="w-full md:h-100 lg:h-120  mt-10 flex justify-center items-center bg-blue-500 gap-5">
        <span class="flex justify-center items-center flex-col">
            <p class="text-3xl font-bold text-[#2A4652]">
            About Gildon Health
            </p>

            <p class="text-xl text-[#2A4652]">
                Gildon Health is dedicated to providing affordable and accessible healthcare solutions to individuals and families.
            </p>
        </span>
        
        <span class="w-full h-[50%] flex justify-center items-center">
            <img src="{{ asset('images/homepage/about.png') }}" class="h-[100%] w-[60%]" alt="About Gildon Health">

        </span>
    </div>
@endsection